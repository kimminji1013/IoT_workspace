...
# AdressBook : 목록 관리
class AddressBookApp(Application):  # 상속
    def __init__(self):
        super().__init__()
    ...
    def destroyed(self):
        super().destroyed() # 부모와 자식 둘 다 destroyed 있음. 이 때 부모의 destroyed 먼저 쓰고싶으면 이 명령어 사용
                            # 이 명령어 사용하지 않는다면 self가 현재 AdressBookApp의 self 이므로 자식것 실행
